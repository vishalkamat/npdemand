...
def est_npdemand(A_u,A_k,beta_u,beta_k,A_obj,worst):
    ...
    # If worst == 1 then do not use data matching restrictions
    ###########################################################################
...
def est_err_npdemand(A_u,A_k,beta_u,beta_k,A_obj):
...
def npdemand(p_a,p_b,P_obs,share,g_a,g_b,g_ab,spec='NPB',K=3,grid_size=5,conf=0,n=0,Sigma='bs',share_b={},level=0.90,incr='auto'):
    ...
    try:
        ...
        temp            = est_err_npdemand(A_u,A_k,beta_u[0],beta_k,A_obj)
        ...
        if incr == 'auto':
            ...
            for i in range(1,B+1):
                try:
                    ...
                    temp            = est_err_npdemand(A_u,A_k,beta_u[i],beta_k,A_obj)
